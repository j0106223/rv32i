...
int main(void) {
    ...
    if(found_flag == 0){
        print("cannot found 7 in the item\n");
    }
    print("searching for number 20\n");
    found_flag = 0;
    for(int i = 0;i < sizeof(item)/sizeof(int);i++){
        if(item[i] == 20){
            print("found number 20 in index");
            itoa(item[i], buffer, 10);
            print(buffer);
            print("\n");
            found_flag = 1;    
    ...
    if(found_flag == 0){
        print("cannot found 20 in the item\n");
    }
    show_element(item, sizeof(item)/sizeof(int));
    finish();
    return 0;
